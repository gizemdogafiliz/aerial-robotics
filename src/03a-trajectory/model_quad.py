...
def setup():
    ...
    # maneuver reads state from our simulator port
    maneuver.connect_port({'local': 'state', 'remote': 'my_state'})

    # nhfc reads state from our simulator port
    nhfc.connect_port({'local': 'state', 'remote': 'my_state'})

    # nhfc reads reference trajectory from maneuver
...
def start():
    ...
    nhfc.set_servo_gain({'gain': {
        'Kpxy': 3, 'Kpz': 5, 'Kqxy': 4, 'Kqz': 0.2,
        'Kvxy': 10, 'Kvz': 8, 'Kwxy': 1, 'Kwz': 0.3,
        'Kixy': 0, 'Kiz': 0
    }})
    ...
    maneuver.set_velocity_limit({'v': 2, 'w': 2})
    maneuver.set_acceleration_limit({'a': 2, 'dw': 2})
    maneuver.set_jerk_limit({'j': 10, 'ddw': 10})
    maneuver.set_snap_limit({'s': 50, 'dddw': 50})
    ...
    # activate continuous reference port tracking
    nhfc.servo(ack=True)
...
#  WAYPOINT SEQUENCE
# ############################
# Same waypoints as 01-intro quadrotor:
...
#  SAVE DATA
# ############################################
LOG_DIR = '/shared-workspace/logs/03a-trajectory/quad'
