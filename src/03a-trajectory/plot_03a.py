...
PLOT_DIR = os.path.join(SCRIPT_DIR, 'plots_03a')

ROBOT = sys.argv[1] if len(sys.argv) > 1 else 'quad'
LOG_DIR = os.path.expanduser(f'~/tk3lab-ws/logs/03a-trajectory/{ROBOT}')
...
# load maneuver log if available
maneuver_path = os.path.join(LOG_DIR, 'maneuver.log')
maneuver_df = None
if os.path.exists(maneuver_path):
    maneuver_df = parse_log(maneuver_path)
    dt_man = maneuver_df['ts'].diff()
    gap_man = dt_man[dt_man > 1.0].index
    if len(gap_man) > 0:
        man_start = gap_man[-1]
        maneuver_df = maneuver_df.iloc[man_start:].reset_index(drop=True)
    maneuver_df['t'] = np.linspace(0, sim_duration, len(maneuver_df))
    print(f"maneuver samples: {len(maneuver_df)}")
...
fig.suptitle(f'Python Simulator + maneuver + nhfc — {ROBOT} (Assignment 3a)', fontsize=14)
...
for c, col, idx in [('ax', 'red', 0), ('ay', 'green', 1), ('az', 'blue', 2)]:
...
fig.savefig(os.path.join(PLOT_DIR, f'plot_03a_{ROBOT}.png'), dpi=150, bbox_inches='tight')
print(f"\nPlot saved to: {PLOT_DIR}/plot_03a_{ROBOT}.png")
