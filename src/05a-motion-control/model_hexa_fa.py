...
# --- setup ----------------------------------------------------------------
def setup():
    state_port = uavpos.state('my_state')

    uavpos.connect_port({'local': 'state', 'remote': 'my_state'})
    uavatt.connect_port({'local': 'state', 'remote': 'my_state'})
...
def start():
    ...
    uavpos.set_saturation({'sat': {'x': 1.5, 'v': 1.5, 'ix': 0}})
    uavpos.set_servo_gain({'gain': {
        'Kpxy': 3, 'Kpz': 6, 'Kvxy': 5, 'Kvz': 8,
        'Kixy': 0, 'Kiz': 0
    }})

    uavatt.set_servo_gain({'gain': {
        'Kqxy': 4, 'Kqz': 3, 'Kwxy': 0.8, 'Kwz': 0.8
    }})

    uavpos.set_emerg({'emerg': {
        'descent': 0.1, 'dx': 5.0, 'dv': 10
    }})
...
# --- rotor_speeds_from_uavatt ---------------------------------------------
def rotor_speeds_from_uavatt(n_act=6):
    desired_speeds = np.zeros(n_act)

    data = uavatt.rotor_input()["rotor_input"]
...
for i, ts in enumerate(tt):
    ...
    speeds = rotor_speeds_from_uavatt()
...
#  SAVE DATA
# ############################################
LOG_DIR = '/shared-workspace/logs/05a-motion-control'
os.makedirs(LOG_DIR, exist_ok=True)

np.savez(os.path.join(LOG_DIR, 'simulation_data.npz'),
         t=t_log, x=x_log, u=u_log, w=w_log, tc=tc_log)
print(f"Saved: {LOG_DIR}/simulation_data.npz")

for f in ['uavpos.log', 'uavatt.log', 'maneuver.log']:
    src = '/tmp/' + f
